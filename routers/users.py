...
def list_mes_users():
    ...
    try:
        cur.execute(
            ...
            FROM mes_users mu
            LEFT JOIN nsh.employees2026_base e
                ON e.employee_code COLLATE utf8mb4_unicode_ci
                 = mu.employee_code COLLATE utf8mb4_unicode_ci
            LEFT JOIN user_roles ur
                ON ur.user_id = mu.id
            LEFT JOIN roles r
                ON r.id = ur.role_id
            GROUP BY mu.id, e.full_name
            ORDER BY mu.id DESC
            """
        )
        rows = cur.fetchall()
...
def get_mes_user(user_id: int):
    ...
    try:
        ...
        if not user:
            raise HTTPException(
                status_code=404,
                detail="Khong tim thay MES user.",
            )
...
def update_mes_user(user_id: int, payload: MesUserUpdate):
    ...
    try:
        cur.execute(
            "SELECT id FROM mes_users WHERE id = %s",
            (user_id,),
        )
        if not cur.fetchone():
            raise HTTPException(
        ...
        if role_ids:
            ...
            if invalid_ids:
                raise HTTPException(
                    status_code=400,
                    detail=(
                        "Role khong hop le hoac da Inactive: "
                        f"{invalid_ids}"
                    ),
                )
